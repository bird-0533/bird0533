...
# PDFの最大文字数（変更可能）
MAX_PDF_CHARS = 50000  # 各PDFあたりの最大文字数

# ====================
# データベース初期化
# ====================
def init_db():
    conn = sqlite3.connect('invite_codes.db')
    c = conn.cursor()
    c.execute('''
    ...
    try:
        ...
        with DDGS() as ddgs:
            for result in ddgs.text(query, max_results=max_results):
                results.append(f"タイトル: {result.get('title', '')}\n内容: {result.get('body', '')}\nURL: {result.get('href', '')}\n")
...
def get_ai_response(question, api_key, context=""):
    ...
    system_prompt = f"あなたは「バード」という名前のAIアシスタントです。\n\n以下の人物の考え方で回答してください：\n\n{MY_THINKING}"
...
# ====================
def main():
    st.set_page_config(page_title="AIアシスタント「バード」", page_icon="🤖")
    
    st.title("🤖 AIアシスタント「バード」")
    
    # APIキー入力
    st.sidebar.header("API設定")
    claude_api_key = st.sidebar.text_input("Claude APIキー", type="password")
    elevenlabs_api_key = st.sidebar.text_input("ElevenLabs APIキー", type="password")
    elevenlabs_voice_id = st.sidebar.text_input("Voice ID", type="password")
    ...
    if not st.session_state.authenticated:
        ...
        invite_code = st.text_input("招待コードを入力してください")
                ...
                if "エラー" not in pdf_text:
                    ...
                    if uploaded_pdf.name not in existing_names:
                        st.session_state.pdf_documents.append({
                            ...
                            "text": pdf_text[:MAX_PDF_CHARS],  # 最大文字数まで
                            "total_pages": total_pages,
                            "total_chars": len(pdf_text)
                        })
                        st.success(f"「{uploaded_pdf.name}」を追加しました（{total_pages}ページ）")
                        st.info(f"読み込み文字数: {min(len(pdf_text), MAX_PDF_CHARS)} / {len(pdf_text)} 文字")
                        st.rerun()
                    else:
    ...
    with st.sidebar.expander("📚 登録済みPDF"):
        if st.session_state.pdf_documents:
            for i, doc in enumerate(st.session_state.pdf_documents):
                ...
                with col1:
                    pages_info = f"（{doc['total_pages']}ページ）" if 'total_pages' in doc else ""
                    st.write(f"📄 {doc['name']} {pages_info}")
                    if 'total_chars' in doc:
                        loaded_chars = min(doc['total_chars'], MAX_PDF_CHARS)
                        st.caption(f"読み込み: {loaded_chars}/{doc['total_chars']}文字")
                with col2:
    ...
    if st.session_state.mode == "text":
        st.subheader("💬 質問を入力")
        st.write("「バード」を含めて質問してください")
        
        if st.session_state.pdf_documents:
            total_chars = sum(min(doc['total_chars'], MAX_PDF_CHARS) for doc in st.session_state.pdf_documents)
            st.info(f"📄 {len(st.session_state.pdf_documents)}個のPDFが読み込まれています（合計{total_chars}文字）")
            ...
            if not user_input:
                ...
                if st.session_state.pdf_documents:
                    ...
                    if st.session_state.selected_category != "すべて":
                        filtered_docs = [doc for doc in st.session_state.pdf_documents 
    ...
    # ==================
        ...
        components.html(speech_recognition_js, height=300)
        
        st.write("---")
        
        recognized_text = st.text_area("認識されたテキスト（編集可能）", st.session_state.recognized_text, height=100)
        use_web_search = st.checkbox("情報がない場合はWeb検索する", value=False)
